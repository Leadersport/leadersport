...
class ControllerCheckoutNicoCheckout extends Controller { 
	...
	public function index() 
	{
		...
		{
			$this->cart->add($_REQUEST['product_id']);
		}
	...
	public function validate($data  = array()) 
	{
		...
		if (isset($_REQUEST['register']) && !empty($_REQUEST['register']) && !$this->customer->isLogged())
		{ 
			$json = array_merge($json, $this->register_validate($data));
		}
		else if (!isset($this->session->data['customer_id'])) $json = $json = array_merge($json, $this->guest_validate($data));
	...
	public function guest_validate() {
		...
		if ($data['opencart_version'] >= 2000)
		{
			...
			if (!$json) {
				...
				if (isset($this->request->post['address_1']) && ((utf8_strlen(trim($this->request->post['address_1'])) < 0) || (utf8_strlen(trim($this->request->post['address_1'])) > 128))) {
					$json['error']['address_1'] = $this->language->get('error_address_1');
				}

				if (isset($this->request->post['city']) && (utf8_strlen(trim($this->request->post['city'])) < 0 || utf8_strlen(trim($this->request->post['city'])) > 128)) {
			...
			if (!$json && $this->request->post) {
				...
				$this->session->data['guest']['postcode'] = $this->request->post['email'];
				//$this->session->data['guest']['email'] = $this->request->post['postcode'];
	...
	//			unset($this->session->data['payment_methods']);
		...
		} else 
		{
			...
			if (!$json) {
				...
				if (isset($this->request->post['address_1']) && ((utf8_strlen($this->request->post['address_1']) < 0) || (utf8_strlen($this->request->post['address_1']) > 128))) {
					$json['error']['address_1'] = $this->language->get('error_address_1');
				}

				if (isset($this->request->post['city']) && (utf8_strlen($this->request->post['city']) < 0 || utf8_strlen($this->request->post['city']) > 128)) {
					$json['error']['city'] = $this->language->get('error_city');
				}
...
/*			$customer_info = $this->model_account_customer->getTotalCustomersByEmail($this->request->post['email']);	
			...
			if ($data['opencart_version'] >= 2000)
			{
				if (!$this->customer->isLogged())
				{
					...
					if (!isset($customer_data['fax'])) $customer_data['fax'] = '';
					if (!isset($customer_data['telephone'])) $customer_data['telephone'] = '';
					if (!isset($customer_data['zone_id'])) $customer_data['zone_id'] = '';
					if (!isset($customer_data['postcode'])) $customer_data['postcode'] = '';
					if (!isset($customer_data['city'])) $customer_data['city'] = '';
					
					$this->session->data['customer_id'] = $customer_id = $this->model_account_customer->addCustomer($customer_data);
					
					if ($this->session->data['customer_id']) $this->customer->login($this->request->post['email'], $this->request->post['password'], true);
	...
	if ($data['opencart_version'] >= 2000) 
		{
				if (!$json) 
				{
					if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing') {
						...
						if (isset($this->request->post['city']) && (utf8_strlen($this->request->post['city']) < 0 || utf8_strlen($this->request->post['city']) > 32)) {
							$json['error']['city'] = $this->language->get('error_city');
						}
						...
						*/ 
						...
						if (!$json) {
							...
							$address_data = array_merge(array('address_2' => '', 'country_id' => '','zone_id' => '','city' => '','postcode' => ''),$this->request->post);//avoid notices for empty fields
		...
		if (!$json) {
			...
			if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing') {
				...
				if (!$json) {
					...
					if ($data['opencart_version'] >= 3000) 
					$this->session->data['payment_address_id'] = $this->model_account_address->addAddress($this->customer->getId(), $address_data);
					else
					$this->session->data['payment_address_id'] = $this->model_account_address->addAddress($address_data);
	...
	public function shipping_address_validate(&$data = array()) {
		...
		if (!$json) {
			...
			if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new') { 
				...
				if (isset($this->request->post['shipping_city']) && (utf8_strlen($this->request->post['shipping_city']) < 0 || utf8_strlen($this->request->post['shipping_city']) > 128)) {
...
/*
		...
		if (isset($country_id) && $country_id)
		{
			...
			$this->session->data['shipping_address']['country_id'] = $this->session->data['shipping_country_id'] = $country_id;
			$this->session->data['shipping_address']['zone_id'] = $this->session->data['shipping_zone_id'] = $zone_id;
			$this->session->data['shipping_address']['postcode'] = $this->session->data['shipping_postcode'] = $postcode;	
		...
		if (empty($shipping_address['country_id'])) $shipping_address['country_id'] = ' ';
		if (empty($shipping_address['zone_id'])) $shipping_address['zone_id'] = ' ';

		if (!isset($this->session->data['shipping_address'])) $this->session->data['shipping_address'] = $shipping_address;
		...
		if ($opencart_version >= 3000)
		{
			...
			else 
			{
				...
				if ($opencart_version >= 2200)
				{
						...
						$extension = 'model_extension_shipping_';
				} else
				{
						$model = 'shipping/';
						$extension = 'model_shipping_';
				}


				foreach ($results as $result) {
					if ($this->config->get($result['code'] . '_status')) {
						$this->load->model($model . $result['code']);

						$quote = $this->{$extension  . $result['code']}->getQuote($this->session->data['shipping_address']);
	...
	public function payment_address($render = true, &$data  = array()) {
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($opencart_version >= 2000) $this->load->language('checkout/checkout'); else $this->language->load('checkout/checkout');
	...
	public function checkout($render = true, &$data  = array()) 
	{
		...
		$data['text_checkout_shipping_address'] = preg_replace('@^.*?\d+:|^.*?%s:@', '', $data['text_checkout_shipping_address']);
	...
	public function cart($render = true, &$data = array()) 
	{
		...
		if ($opencart_version >= 2000) $this->load->language('checkout/cart'); else $this->language->load('checkout/cart');
			...
			if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
				...
				foreach ($products as $product) {
					...
					if ($product['image']) {
						...
						if (!$width) $width = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_width');

						$height = $this->config->get($this->config->get('config_theme') . '_image_cart_height');
						if (!$height) $height = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_height');
				...
				$data['checkout'] = $this->url->
			link('checkout/nicocheckout', '', true);
				...
				if ($files) {
					foreach ($files as $file) {
						$result = $this->load->controller($extension_path . 'total/' . basename($file, '.php'));
		...
		} else
		{
            ...
            foreach ($products as $product) {
                ...
                if ($product['image']) {
					...
					$width = $this->config->get($this->config->get('config_theme') . '_image_cart_width');
					if (!$width) $width = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_width');
					if (!$width) $width = $this->config->get('config_image_cart_width');

					$height = $this->config->get($this->config->get('config_theme') . '_image_cart_height');
					if (!$height) $height = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_cart_height');
					if (!$height) $height = $this->config->get('config_image_cart_height');
...
$this->load->model('tool/image');
  ...
  if ($product['image']) {
				...
				if ($render !== false)
				{
					...
					echo $data['payment'] = $this->load->controller($extension_path . 'payment/' . $this->session->data['payment_method']['code']);
				}
