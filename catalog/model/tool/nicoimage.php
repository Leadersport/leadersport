...
class ModelToolNicoImage extends Model {
	/**
	*	
	...
	*/
	...
	function resizewidth($filename, $width, $height) {
		...
		$old_image = $filename;
		$new_image = 'cache/' . substr($filename, 0, strrpos($filename, '.')) . '-wd-' . $width . 'x' . $height . '.' . $extension;
		...
		if (!file_exists(DIR_IMAGE . $new_image) || (filemtime(DIR_IMAGE . $old_image) > filemtime(DIR_IMAGE . $new_image))) {
			...
			$image = new Image(DIR_IMAGE . $old_image);
			$image->resizewidth($width, $height);
			$image->save(DIR_IMAGE . $new_image);
		}
	...
	function resizeheight($filename, $width, $height) {
		...
		$old_image = $filename;
		$new_image = 'cache/' . substr($filename, 0, strrpos($filename, '.')) . '-he-' . $width . 'x' . $height . '.' . $extension;
		...
		if (!file_exists(DIR_IMAGE . $new_image) || (filemtime(DIR_IMAGE . $old_image) > filemtime(DIR_IMAGE . $new_image))) {
			...
			$image = new Image(DIR_IMAGE . $old_image);
			$image->resizeheight($width, $height);
			$image->save(DIR_IMAGE . $new_image);
		}
