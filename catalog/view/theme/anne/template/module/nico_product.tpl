...
if (!function_exists('nico_product')) {
    ...
    ?>
    <div class="product" data-product-id="<?php echo $product['product_id']; ?>">
	  ...
	  <?php }
	   ...
	   if (isset($product['viewed']) && $product['viewed'] && ($product['viewed'] >= $most_viewed)) { ?>
		...
		<div class="price<?php if ($product['special']) { ?> price_sale<?php }?>">
		  ...
		  <?php } else { ?>
		 <p> <span class="price-new"><?php echo $product['special']; ?> </span> <span class="price-old"><?php echo $product['price']; ?></span></p>
		  <?php } ?>
		</div>
		<?php } ?>
		...
		<a class="add-to-cart btn" onclick="return addToCart('<?php echo $product['product_id']; ?>'<?php if (isset($product['minimum'])) {?>, '<?php echo $product['minimum']; ?>'<?php }?>);"><?php echo $_button_cart; ?><span class="cart-icon-mini"></span></a>
		</div>
		</div>
		<div class="prod-hover-right">
		
		<div class="prod-hover-compare">
		<a class="compare-button" data-toggle="tooltip" title="В сравнение" onclick="compare.add('<?php echo $product['product_id']; ?>');"></a>
		</div>
		
		<div class="prod-hover-wish">
		<a class="whish-button" data-toggle="tooltip" title="В закладки" onclick="wishlist.add('<?php echo $product['product_id']; ?>');"></a>
		</div>
