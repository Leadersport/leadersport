...
<div class="navbar yamm <?php if (isset($grid_cols) &&$grid_cols) {?>col-md-<?php echo $grid_cols;} else echo '';?> <?php if (isset($grid_offset) && $grid_offset) {?>col-md-offset-<?php echo $grid_offset;}?> <?php if (isset($grid_padding) && $grid_padding) {?>no_padding<?php if (isset($grid_padding) && $grid_padding == 2) echo 'left';else if (isset($grid_padding) && $grid_padding == 3) echo 'right'; }?><?php if (isset($hide_on_mobile) && $hide_on_mobile == 'true') {?> hide_on_mobile<?php }?>">
...
<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
 ...
 <div class="icons-bar">
	...
	<?php if (isset($mobile_text) && $mobile_text) {?><div class="navbar-brand visible-xs"><?php echo $mobile_text;?></div><?php }?>
  </div>
</button>
	<?php 
	
	if (isset($session) && isset($session->data['language'])) 
	{
	 ...
	 <div class="navbar-collapse collapse">
		<ul class="nav navbar-nav">
			...
			<li class="<?php if ($category['menu_type'] != 'text' && $category['full_width'] == 1) {?>mega<?php }?>
				...
				<?php if ($level == 1){  if ((isset($category['children']) || $category['menu_type'] == 'category') && $_type != 'vertical') {?><b class="fa fa-caret-down"></b><?php } }?>
				</a>
							...
							<div class="row">
								...
								foreach ($categs as $cat) { if (is_array($cat['children']) && count($cat['children'])) {$i++;?>
								  <ul class="col-sm-<?php echo $_per_row;?> list-unstyled">
									<li>
									  <p class="main-cat"><a href="<?php echo $cat['url'];?>"><strong><?php echo $cat['name'];?></strong></a><?php if ($cat['thumb']) {?><img class="cat_thumb" src="<?php echo $cat['thumb'];?>" alt=""><?php }?></p>
									</li>
				...
				<?php } else if ($category['menu_type'] == 'product') {;?> 
					<ul class="dropdown-menu <?php if (isset($category['column']) && $category['column'] && $category['column'] > 1) { echo 'multi-' . $category['column'] . '-columns'; }?>">
						<li>
						  ...
						  <div class="yamm-content clearfix">
							<div class="row">
							<?php if (isset($category['autocomplete']) && $products = $__this->get_products($category['autocomplete']))
