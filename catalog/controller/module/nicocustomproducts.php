...
class ControllerModuleNicocustomproducts extends NicoModule
{
	...
	public function index($setting) 
	{
		...
		$key = 'nicocustomproducts.'. (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' .  crc32(isset($setting['name'])?$setting['name']:'-') . '.' . $currency_code . '.' . $this->config->get('config_customer_group_id') . md5($_SERVER['REQUEST_URI']) . $_nico_module_counter['nicocustomproducts'];
