...
global $opencart_version, $ocmods_ignore, $opencart_major_version;
$opencart_version = (int)str_replace('.','',VERSION);
$opencart_major_version = (int)substr(str_replace('.','',VERSION), 0, 2);
...
class ControllerModuleNicoimport extends Controller
{
	...
	function refresh_modifications()
	{
		//Log
		$log = array();
...
*/
		...
		global $db, $opencart_version, $opencart_major_version;
		...
		$ocmods_dir = DIR_TEMPLATE . $theme . '/nico_theme_editor/ocmods/' . $opencart_major_version . '00/';
		if (!is_dir($ocmods_dir)) $ocmods_dir = DIR_TEMPLATE . $theme . '/nico_theme_editor/ocmods/3000/';
		$this->sendChunk('Reading ocmods from ' . $ocmods_dir);
		...
		{
			$css_content = '';
			foreach($data['styles'] as $css_file)
			{
				$css_file = str_replace('catalog/', '', $css_file);
				$css_content .= file_get_contents(DIR_APPLICATION . $css_file) . "\n\n\n";
				$this->sendChunk('Downloading style - ' . $css_file);
			}
			
			$editor_css_file = DIR_TEMPLATE . $theme . '/css/editor_settings.css';
			@chmod($editor_css_file, 0777);
			file_put_contents($editor_css_file, $css_content);
			$this->sendChunk('Writing styles to ' . $editor_css_file);
		}
